...
def runglowaurora(eflux,e0,t0,glat,glon,z_km=None,
...
#%% (2) msis,iri,glow model
    ion,ecalc,photI,ImpI,isr,prate,lrate,UV = glowfort.aurora(z_km,yd,utsec,glat,glon%360,
...
#%% handle the outputs including common blocks
    ...
    zceta = DataArray(dims=['z_km','wavelength_nm','type'],
                      coords={'z_km':z_km, 'wavelength_nm':lamb[:11]},
...
#%% production and loss rates
    ...
    prates = DataArray(prate[...,:12], #columns 12:20 are identically zero
    ...
    lrates = DataArray(lrate[...,:12], #columns 12:20 are identically zero
                      dims=['type','z_km','reaction'],
                      coords={'type':['pre','final'],
                               'z_km':z_km,
                        'reaction':['O+(2P)','O+(2D)','O+(4S)','N+','N2+','O2+','NO+',
    ...
    sion = DataArray(glowfort.cglow.sion,
                     dims=['gas','z_km'],
                     coords={'gas':['O','O2','N2'],
                             'z_km':z_km})
# %% cleanup
...
def makeeigen(EK,diffnumflux,T,glatlon,makeplot=[None],odir=None,zlim=None):
    ...
    ver = None
    if len(T) > 1:
       logging.error('more than one time not yet implemented. You will get last time')
