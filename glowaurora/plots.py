...
def _plotisr(params, sim, st: str):
    ...
    if 'photIon' not in sim or 'isr' not in sim:
        return
    ...
    ax.semilogx(sim['photIon'].loc[:, ind], sim.z_km)
    ...
    ax.semilogx(sim['isr'].loc[:, ind], sim.z_km)
