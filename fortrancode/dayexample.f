...
C ICONJ   conjugate photoelectron flux switch: 0=off, 1=on
...
C
      read (5,*) idate,ut,glat,glong,f107a,f107,f107p,ap
C
...
C
C
C Fill altitude array and initialize and N(2D):
C
      DO J=1,JMAX
        ZZ(J)   = Z(J) * 1.E5
        ZND(J)  = 0.
...
C
C
C Put zero electron flux into PHITOP array:
C
      DO N=1,NBINS
        PHITOP(N) = 0.0
      END DO
