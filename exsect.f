...
C
      ...
      use cxglow,only: NMAJ,NEI,NBINS
      use cxglow,only: WW,AO,OMEG,ANU,BB,AUTO,THI,AK,AJ,TS,TA,TB,GAMS,  ! /CXPARS/
     |  GAMB
      use cxglow,only: SIGS,PE,PI=>PIN,SIGEX,SIGIX,SIGA,SEC,SIGA,IIMAXX ! /CXSECT/
...
!     write(6,"('Enter exsect: nbins=',i4,' ENER=',/,(8es12.4))")
!    |  nbins,ener
!     write(6,"('Enter exsect: nbins=',i4,' DEL=',/,(8es12.4))")
!    |  nbins,del
      ...
      NUM  = (/31,28,28/)
!
! Set arrays in module cxglow (formerly COMMON/CXSECT/)
! (all are dimensioned (nei,nmaj) (10,3))
! (data statements are not allowed for use-associated arrays, 
!  so must use runtime array constructors)
!
      WW(1:nei,1)=
...
!        write(6,"('exsect: ij=',i4,' iv=',i4,' ii=',i4,' EX=',es12.4,
!    |     ' EC(II,IJ)=',es12.4)") ij,iv,ii,ex,ec(ii,ij)
...
!         write(6,"('exsect: ij=',i4,' iv=',i4,' sigs(ij,iv)=',es12.4)")
!    |      ij,iv,sigs(ij,iv)
...
C
      ...
      use cxglow,only: THI,AK,AJ,TS,TA,TB,GAMS,GAMB ! /CXPARS/
      implicit none
!
! Args:
